...
bool GameScreen2::init()
{
	...
	btnresume->addTouchEventListener(CC_CALLBACK_1(GameScreen2::GoToPauseScene, this));
	...
	ghost->setPosition((Vec2(visibleSize.width * 1.5, visibleSize.height * 0.55)));
...
void GameScreen2::update(float dt) {
	...
	Rect signe1 = knz->getBoundingBox();

	if (joueur1.intersectsRect(signe1))
	{
	...
	if (floor->getPosition().x < -2000.0f) {
		auto scene = GameOver::createScene();
		Director::getInstance()->replaceScene(scene); // mbghatx tkhdm !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
	}
...
void GameScreen2::buttonPressed() {
	Scene* scene = GameScreen2::createScene();
